...
required for [Story 06](stories/06-approval-github-pr.md) PRs) with Authentik federation in
...
Refs: [overview](overview.md), [Story 07](stories/07-handoff-moodle-publication.md).
...
[Story 07](stories/07-handoff-moodle-publication.md). If undefined → no value delivered.
...
- [Story 06](stories/06-approval-github-pr.md): merge → **GitHub Action** → Moodle (automatic).
- [Story 07](stories/07-handoff-moodle-publication.md): `COURSE_MASTER_PLAN` → **Sheet** →
...
**Risk:** PR authorship/review ([Story 06](stories/06-approval-github-pr.md)) is GitHub-based;
...
- **Numbering** — canonicalized to 0–10 in this folder; source had two "Story 6" and an
  off-by-one. Jira (`ENG-27x`) is aspirational and should inherit from here.
- **Story 02 confirmation email** marked "Optional" — confirms nothing. Decide.
- **Story 06 companion `.yml` vs Markdown** for category/cohort/shortname — TBD.
...
- **Story 10** is a stub owned by Ivan; **Story 09** Canva→Metricool automation is WIP.
